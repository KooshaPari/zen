...
from tools.agent_doctor import AgentDoctorTool  # noqa: E402
from tools.agent_inbox import AgentInboxTool  # noqa: E402
from tools.agent_registry import AgentRegistryTool  # noqa: E402
from tools.models import ToolOutput  # noqa: E402

# XML Communication Protocol imports
from tools.shared.agent_models import AgentType  # noqa: E402
from tools.universal_executor import DeployTool  # noqa: E402
from utils.agent_prompts import enhance_agent_message  # noqa: E402
...
ESSENTIAL_TOOLS = {"version", "listmodels", "chat"}
...
# Streamlined tool registry with essential operational tools only
# Deploy tool consolidates all execution capabilities (chat, thinkdeep, planner, consensus, codereview,
# analyze, refactor, testgen, secaudit, docgen, tracer, precommit, challenge, fastapply_edit)
TOOLS = {
    # Core execution - unified interface for all capabilities
    "deploy": DeployTool(),  # Unified execution interface (LLM, agents, workflows, tools, batch, streaming)

    # Essential operational tools
    "agent_registry": AgentRegistryTool(),  # Discover and describe available AgentAPI-supported CLI agents
    "agent_inbox": AgentInboxTool(),  # Check status and retrieve results from async agent tasks
    "listmodels": ListModelsTool(),  # List all available AI models by provider
    "version": VersionTool(),  # Display server version and system information
    "debug": DebugIssueTool(),  # Root cause analysis and debugging assistance
    "agent_doctor": AgentDoctorTool(),  # Diagnose AgentAPI and environment

    # Individual core tools explicitly exposed for simulator compatibility
    "chat": ChatTool(),
    "analyze": AnalyzeTool(),
...
PROMPT_TEMPLATES = {
    ...
    "universal_executor": {
        ...
        "description": "Universal execution interface supporting all modes: LLM, agent, Aegis workflows, and MCP tools",
        "template": "Execute with {model} using {agent_type} mode",
    },
}
...
def configure_providers():
    ...
    # Load persisted batches on startup
    try:
        from utils.batch_registry import load_batches_from_disk
        load_batches_from_disk()
        logger.info("Loaded persisted batch registry from disk")
    except Exception:
        logger.debug("No persisted batch registry found or failed to load")

    # Persist batches on shutdown
    def _persist_batches_on_exit():
        try:
            from utils.batch_registry import save_batches_to_disk
            save_batches_to_disk()
        except Exception:
            pass
    atexit.register(_persist_batches_on_exit)
...
async def handle_list_tools() -> list[Tool]:
    ...
    for tool in TOOLS.values():
        ...
        _schema = tool.get_input_schema() or {"type": "object", "properties": {}, "required": []}
        try:
            props = dict(_schema.get("properties", {}))
            req = list(_schema.get("required", []))
            if "work_dir" not in props:
                props["work_dir"] = {
                    "type": "string",
                    "description": "REQUIRED: Repo-relative working directory that scopes data access"
                }
            if "work_dir" not in req:
                req.append("work_dir")
            _schema["properties"] = props
            _schema["required"] = req
        except Exception:
            pass
        tools.append(
            Tool(
    ...
    # Ensure core compatibility tools are listed even if not in TOOLS (fallback)
    if "chat" not in [t.name for t in tools]:
        try:
            chat_tool = ChatTool()
            ...
            tools.append(
                Tool(
                    ...
                    inputSchema=chat_tool.get_input_schema(),
                    annotations=tool_annotations,
                )
            )
        except Exception:
            logger.debug("Failed to add fallback ChatTool to tool list", exc_info=True)
...
async def handle_call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
    ...
    enforce = os.getenv("ZEN_ENFORCE_WORKDIR", "0").lower() in ("1", "true", "yes")
    ...
    if "_scope_context" in arguments and isinstance(arguments["_scope_context"], ScopeContext):
        ...
        agent_id = os.getenv("ZEN_AGENT_ID", "zen-mcp-server")
        session_id = arguments.get("continuation_id") or str(uuid.uuid4())
        ...
        scope_context.org_id = os.getenv("ZEN_ORG_ID", "default")
        scope_context.project_id = os.getenv("ZEN_PROJECT_ID", "default")
        scope_context.source_tool = name

    try:
        norm_rel, abs_path = validate_and_normalize_work_dir(work_dir)
        scope_context.work_dir = norm_rel
    ...
    if name in TOOLS:
        ...
        if not tool.requires_model():
            ...
            # Execute tool directly without model context, but still normalize result for simulator
            raw_result = await tool.execute(arguments)
            try:
                from utils.mcp_normalize import normalize_tool_result
                result = normalize_tool_result(name, arguments, raw_result)
                return result
            except Exception:
                logger.debug("[MCP_NORMALIZE] Failed to normalize non-model tool result; returning raw result", exc_info=True)
                return raw_result
        ...
        # Execute tool with XML communication protocol enhancement
        enhanced_arguments = arguments.copy()

        # Enhance prompts with XML communication protocol for AI-powered tools
        for field in ["prompt", "question", "code", "message", "text"]:
            if field in enhanced_arguments and isinstance(enhanced_arguments[field], str):
                original_input = enhanced_arguments[field]
                # Default to Claude agent type for STDIO transport
                enhanced_input = enhance_agent_message(original_input, AgentType.CLAUDE)
                enhanced_arguments[field] = enhanced_input
                logger.debug(f"Enhanced {field} with XML communication protocol")
                break

        # Execute tool with enhanced arguments
        raw_result = await tool.execute(enhanced_arguments)
        logger.info(f"Tool '{name}' execution completed")

        # Preserve JSON ToolOutput if already present; otherwise normalize to include continuation_id
        try:
            from utils.mcp_normalize import normalize_tool_result
            result = normalize_tool_result(name, arguments, raw_result)
        except Exception:
            logger.debug("[MCP_NORMALIZE] Failed to normalize tool result; returning raw result", exc_info=True)
            result = raw_result
        ...
        # Ensure non-empty JSON text with continuation metadata
        try:
            # Avoid importing TextContent with the same name inside the function to prevent scoping issues
            ...
            if isinstance(result, list) and result and hasattr(result[0], "text"):
                text0 = result[0].text or ""

            # INFO log for diagnostics
            is_json = False
            try:
                _tmp = json.loads(text0) if text0 else None
                is_json = isinstance(_tmp, (dict, list))
            except Exception:
                is_json = False
            logger.info(f"[MCP] Tool '{name}' outgoing first content is_json={is_json} len={len(text0)}")

            if not is_json:
                # If non-JSON on initial calls, synthesize a ToolOutput so simulator can parse continuation_id
                try:
                    from tools.models import ToolOutput
                    from utils.conversation_memory import create_thread
                    cont_id = arguments.get("continuation_id") or create_thread(tool_name=name, initial_request=arguments)
                    wrapper = ToolOutput(
                    ...
                    result = [MCPTextContent(type="text", text=wrapper.model_dump_json())]
                    logger.info(f"[MCP] Synthesized ToolOutput wrapper with continuation_id={cont_id}")
                except Exception:
                    logger.debug("[MCP] Wrapper synthesis failed", exc_info=True)
        except Exception:
            logger.debug("[MCP] Post-normalization backstop failed", exc_info=True)

        # Debug: log outgoing first content text snippet for simulator parsing
        try:
            if result and isinstance(result, list) and len(result) > 0:
                ...
                try:
                    ...
                    is_json = False
                logger.debug(f"[MCP_DEBUG] Outgoing first content is_json={is_json} len={len(text0)} snippet={text0[:200]}")
        except Exception:
            pass
        # Final guard: ensure initial responses always return JSON with continuation_id
        try:
            if not arguments.get("continuation_id"):
                from mcp.types import TextContent as MCPTextContent
                ...
                if not first_text.strip():
                    ...
                    try:
                        _ = json.loads(first_text)
                        # If JSON is valid, keep as-is
                    except Exception:
                        needs_wrap = True
                if needs_wrap:
                    cont_id = create_thread(tool_name=name, initial_request=arguments)
                    wrapper = ToolOutput(
                        status="continuation_available",
                        content=first_text if first_text else "",
                        content_type="text",
                        continuation_offer={
                    ...
                    # Also include top-level continuation_id for simulator compatibility
                    wrapper_dict = wrapper.model_dump()
                    wrapper_dict["continuation_id"] = cont_id
                    result = [MCPTextContent(type="text", text=json.dumps(wrapper_dict))]
                    logger.info(
                        f"[MCP] Final guard synthesized wrapper with continuation_id={cont_id} for non-JSON/empty initial response"
                    )
        except Exception:
            logger.debug("[MCP] Final guard failed to synthesize wrapper", exc_info=True)

        return result

    # Handle unknown tool requests gracefully with fallback for core tools
    else:
        # Fallback: if chat not listed, instantiate directly and normalize output
        if name == "chat":
            try:
                tool = ChatTool()
                raw_result = await tool.execute(arguments)

                # Normalize using the shared normalizer to guarantee JSON ToolOutput/continuation_offer
                try:
                    from utils.mcp_normalize import normalize_tool_result
                    result = normalize_tool_result(name, arguments, raw_result)
                except Exception:
                    logger.debug("[MCP_NORMALIZE][fallback] Failed to normalize tool result; returning raw result", exc_info=True)
                    result = raw_result

                # Debug for simulator parsing
                try:
                    if result and isinstance(result, list) and len(result) > 0:
                        content0 = result[0]
                        text0 = getattr(content0, "text", "") if hasattr(content0, "text") else ""
                        ...
                        try:
                            ...
                            is_json = False
                        logger.debug(f"[MCP_DEBUG][fallback] Outgoing first content is_json={is_json} len={len(text0)} snippet={text0[:200]}")
                except Exception:
                    pass

                return result
            except Exception as e:
                logger.error(f"Fallback ChatTool execution failed: {e}")
                return [TextContent(type="text", text=f"Error executing chat: {e}")]
        # Ensure simulator can parse continuation_id even on unknown tools
        try:
            from tools.models import ToolOutput
            from utils.conversation_memory import create_thread
            cont_id = create_thread(tool_name=name, initial_request=arguments)
            wrapper = ToolOutput(
                ...
                continuation_offer={
                    "continuation_id": cont_id,
                    "note": "Continue using this continuation_id",
                    "remaining_turns": 9,
                },
                metadata={"tool_name": name},
            )
